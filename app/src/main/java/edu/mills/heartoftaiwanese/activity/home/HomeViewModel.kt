...
class HomeViewModel : HomeContract.HomeViewModel, BaseViewModel() {
    ...
    override fun fetchChinese(english: String) {
        GlobalScope.launch {
            ...
            when {
                result.isTaiwanese -> throw IllegalStateException("Should get Chinese, but found Taiwanese!")
                result.resultCode == WebResultCode.RESULT_OK -> result.chinese?.let {
                    view.onChineseFetched(it)
    ...
    override fun fetchTaiwanese(chinese: String) {
        GlobalScope.launch {
            ...
            when {
                result.isChinese -> throw IllegalStateException("Should get Taiwanese, but found Chinese!")
                result.resultCode == WebResultCode.RESULT_OK -> result.taiwanese?.let {
